...
namespace Arbor
{
	...
	public sealed class State : Node, INodeBehaviourContainer
	{
		...
		private class CallbackEntry
		{
			...
			public StateBehaviour[] onUpdateBehaviours = null;
			public StateBehaviour[] onLateUpdateBehaviours = null;
			public StateBehaviour[] onFixedUpdateBehaviours = null;
			...
			public void Update(List<Object> behaviours)
			{
				...
				this.onUpdateBehaviours = null;
				this.onLateUpdateBehaviours = null;
				this.onFixedUpdateBehaviours = null;

				int behaviourCount = behaviours.Count;
				if (behaviourCount > 0)
				{
					List<StateBehaviour> enableBehaviours = new List<StateBehaviour>();
					List<StateBehaviour> onUpdateBehaviours = new List<StateBehaviour>();
					List<StateBehaviour> onLateUpdateBehaviours = new List<StateBehaviour>();
					List<StateBehaviour> onFixedUpdateBehaviours = new List<StateBehaviour>();
					...
					for (int behaviourIndex = 0; behaviourIndex < behaviourCount; behaviourIndex++)
					{
						...
						if (callbackInfo.hasUpdate)
						{
							onUpdateBehaviours.Add(behaviour);
						}

						if (callbackInfo.hasLateUpdate)
						{
							onLateUpdateBehaviours.Add(behaviour);
						}

						if (callbackInfo.hasFixedUpdate)
						{
							onFixedUpdateBehaviours.Add(behaviour);
						}
					}
					...
					if (onUpdateBehaviours.Count > 0)
					{
						this.onUpdateBehaviours = onUpdateBehaviours.ToArray();
					}
					if (onLateUpdateBehaviours.Count > 0)
					{
						this.onLateUpdateBehaviours = onLateUpdateBehaviours.ToArray();
					}
					if (onFixedUpdateBehaviours.Count > 0)
					{
						this.onFixedUpdateBehaviours = onFixedUpdateBehaviours.ToArray();
					}
				}
		...
		internal State(ArborFSMInternal stateMachine, int nodeID, bool resident) : base(stateMachine, nodeID)
		{
			_Resident = resident;
		}
		...
		internal void Activate(bool enable)
		{
			...
			if (enable)
			{
				...
				if (_BreakPoint && Application.isEditor)
				{
					Debug.Break();
				}
			}
		...
		internal void UpdateBehaviours()
		{
			...
			UpdateCallbackEntry();
			StateBehaviour[] behaviours = _CallbackEntry.onUpdateBehaviours;
			if (behaviours == null)
			{
				return;
			}
		...
		internal void LateUpdateBehaviours()
		{
			...
			UpdateCallbackEntry();
			StateBehaviour[] behaviours = _CallbackEntry.onLateUpdateBehaviours;
			if (behaviours == null)
			{
				return;
			}
		...
		internal void FixedUpdateBehaviours()
		{
			...
			UpdateCallbackEntry();
			StateBehaviour[] behaviours = _CallbackEntry.onFixedUpdateBehaviours;
			if (behaviours == null)
			{
				return;
			}
