...
namespace ArborEditor
{
	...
	internal sealed class AnimatorParameterReferencePropertyDrawer : PropertyDrawer
	{
		Rect DoGUI(Rect position, SerializedProperty property, GUIContent label, bool isDraw)
		{
			...
			FlexibleSceneObjectProperty animatorProperty = new FlexibleSceneObjectProperty(property.FindPropertyRelative("_Animator"));
			...
			lineRect.height = EditorGUI.GetPropertyHeight(animatorProperty.property);

			if (isDraw)
			{
				EditorGUI.PropertyField(lineRect, animatorProperty.property);
			}
			...
			Animator animator = animatorProperty.constantValue as Animator;
